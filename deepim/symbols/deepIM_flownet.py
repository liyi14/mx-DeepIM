...
class deepIM_flownet(Symbol):
    ...
    def get_convs(self, data_iter, big_cfg, small_cfg, iter_idx):
        ...
        flow_conv1 = mx.symbol.Convolution(
            name="flow_conv1", data=data, num_filter=64, pad=(3, 3), kernel=(7, 7), stride=(2, 2), no_bias=False
        )
        ReLU1 = mx.symbol.LeakyReLU(name="ReLU1", data=flow_conv1, act_type="leaky", slope=0.1)
        # scale 4
        conv2 = mx.symbol.Convolution(
            name="conv2", data=ReLU1, num_filter=128, pad=(2, 2), kernel=(5, 5), stride=(2, 2), no_bias=False
        )
        ReLU2 = mx.symbol.LeakyReLU(name="ReLU2", data=conv2, act_type="leaky", slope=0.1)
        # scale 8
        conv3 = mx.symbol.Convolution(
            name="conv3", data=ReLU2, num_filter=256, pad=(2, 2), kernel=(5, 5), stride=(2, 2), no_bias=False
        )
        ReLU3 = mx.symbol.LeakyReLU(name="ReLU3", data=conv3, act_type="leaky", slope=0.1)
        conv3_1 = mx.symbol.Convolution(
            name="conv3_1", data=ReLU3, num_filter=256, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False
        )
        ReLU4 = mx.symbol.LeakyReLU(name="ReLU4", data=conv3_1, act_type="leaky", slope=0.1)
        # scale 16
        conv4 = mx.symbol.Convolution(
            name="conv4", data=ReLU4, num_filter=512, pad=(1, 1), kernel=(3, 3), stride=(2, 2), no_bias=False
        )
        ReLU5 = mx.symbol.LeakyReLU(name="ReLU5", data=conv4, act_type="leaky", slope=0.1)
        conv4_1 = mx.symbol.Convolution(
            name="conv4_1", data=ReLU5, num_filter=512, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False
        )
        ReLU6 = mx.symbol.LeakyReLU(name="ReLU6", data=conv4_1, act_type="leaky", slope=0.1)
        # scale 32
        conv5 = mx.symbol.Convolution(
            name="conv5", data=ReLU6, num_filter=512, pad=(1, 1), kernel=(3, 3), stride=(2, 2), no_bias=False
        )
        ReLU7 = mx.symbol.LeakyReLU(name="ReLU7", data=conv5, act_type="leaky", slope=0.1)
        conv5_1 = mx.symbol.Convolution(
            name="conv5_1", data=ReLU7, num_filter=512, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False
        )
        ReLU8 = mx.symbol.LeakyReLU(name="ReLU8", data=conv5_1, act_type="leaky", slope=0.1)
        # scale 64
        conv6 = mx.symbol.Convolution(
            name="conv6", data=ReLU8, num_filter=1024, pad=(1, 1), kernel=(3, 3), stride=(2, 2), no_bias=False
        )
        ReLU9 = mx.symbol.LeakyReLU(name="ReLU9", data=conv6, act_type="leaky", slope=0.1)
        conv6_1 = mx.symbol.Convolution(
            name="conv6_1", data=ReLU9, num_filter=1024, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False
        )
        ...
        fc6 = mx.symbol.FullyConnected(name="fc6", data=flatten_0, num_hidden=256, no_bias=False)
        relu_fc6 = mx.symbol.LeakyReLU(name="ReLU11", data=fc6, act_type="leaky", slope=0.1)

        fc7 = mx.symbol.FullyConnected(name="fc7", data=relu_fc6, num_hidden=256, no_bias=False)
        relu_fc7 = mx.symbol.LeakyReLU(name="ReLU12", data=fc7, act_type="leaky", slope=0.1)
        ...
        if big_cfg.network.PRED_FLOW or big_cfg.network.PRED_MASK:
            ...
            Convolution1 = mx.symbol.Convolution(
                name="Convolution1", data=ReLU10, num_filter=2, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False
            )  # (B,2,8,10)
            # scale 32
            deconv5 = mx.symbol.Deconvolution(
                name="deconv5", data=ReLU10, num_filter=512, pad=(0, 0), kernel=(4, 4), stride=(2, 2), no_bias=False
            )  # (B,512, 18, 22)
            crop_deconv5 = mx.symbol.Crop(name="crop_deconv5", *[deconv5, ReLU8], offset=(1, 1))  # (B,512,15,20)
            ...
            crop_upsampled_flow6_to_5 = mx.symbol.Crop(
                name="crop_upsampled_flow6_to_5", *[upsample_flow6to5, ReLU8], offset=(1, 1)
            )  # (B,2,15,20)
            Concat2 = mx.symbol.Concat(name="Concat2", *[ReLU8, ReLU11, crop_upsampled_flow6_to_5])  # (B, 1026,15,20)
            Convolution2 = mx.symbol.Convolution(
                name="Convolution2", data=Concat2, num_filter=2, pad=(1, 1), kernel=(3, 3), stride=(1, 1), no_bias=False
            )  # (B,2,15,20)
            # scale 16
            deconv4 = mx.symbol.Deconvolution(
                name="deconv4", data=Concat2, num_filter=256, pad=(0, 0), kernel=(4, 4), stride=(2, 2), no_bias=False
            )  # (B,256,32,42)
            ...
            crop_upsampled_flow5_to_4 = mx.symbol.Crop(
                name="crop_upsampled_flow5_to_4", *[upsample_flow5to4, ReLU6], offset=(1, 1)
            )  # (B,2,32,42)
            Concat3 = mx.symbol.Concat(name="Concat3", *[ReLU6, ReLU12, crop_upsampled_flow5_to_4])  # (B,770,30,40)
            feat_list[1] = Concat3

        return feat_list

    def get_loss(self, big_cfg, small_cfg, conv_feat, deconv_feat, labels, group_list, regressor_idx):
        pred = {}
        ...
        if big_cfg.network.PRED_FLOW:
            ...
            flow_est_crop = mx.symbol.Crop(
                *[flow_est_resize, labels["image_observed"]], offset=(8, 8), name="flow_est_crop"
            )
            flow_loss_ = labels["flow_weights"] * mx.sym.square(
                data=(flow_est_crop - labels["flow"] / big_cfg.dataset.NORMALIZE_FLOW), name="flow_loss"
            )

            flow_loss = mx.sym.MakeLoss(name="flow_loss", data=flow_loss_, grad_scale=small_cfg.LW_FLOW / (480 * 640))
        ...
        if big_cfg.network.REGRESSOR_NUM == 1:
            ...
            zoom_trans_est = mx.symbol.FullyConnected(name="trans", data=conv_feat, num_hidden=3, no_bias=False)
        else:
            rot_est = mx.symbol.FullyConnected(name="rot", data=conv_feat, num_hidden=4, no_bias=False)
            zoom_trans_est = mx.symbol.FullyConnected(name="trans", data=conv_feat, num_hidden=3, no_bias=False)
        ...
        if small_cfg.SE3_DIST_LOSS:
            ...
            rot_loss_ = 1 - mx.sym.square(
                mx.sym.batch_dot(rot_gt_dot, rot_est_norm_dot, transpose_b=True), name="rot_loss"
            )

            rot_loss = mx.sym.MakeLoss(name="rot_loss", data=rot_loss_, grad_scale=small_cfg.LW_ROT)
            ...
            if small_cfg.TRANS_LOSS_TYPE == "L2":
                ...
                trans_loss_ = mx.sym.smooth_l1(
                    data=(zoom_trans_est - zoom_trans_gt), name="trans_loss", scalar=small_cfg.TRANS_SMOOTH_L1_SCALAR
                )
            ...
            trans_loss = mx.sym.MakeLoss(name="trans_loss", data=trans_loss_, grad_scale=small_cfg.LW_TRANS)
        ...
        if small_cfg.SE3_PM_LOSS:
            ...
            if small_cfg.SE3_PM_LOSS_TYPE == "L1":
                point_matching_loss_all = mx.sym.abs(
                    data=(point_cloud_observed_est - point_cloud_observed) / norm_term, name="point_matching_loss_all_"
                )
        ...
        if big_cfg.network.PRED_MASK:
            ...
            mask_pred_resize_crop = mx.symbol.Crop(
                *[mask_pred_resize, labels["image_observed"]], offset=(8, 8), name="mask_pred_resize_crop"
            )
            ...
            group_list.append(mx.sym.BlockGrad(labels["mask_gt_observed"], name="zoom_mask_gt_observed"))
    ...
    def get_train_symbol(self, cfg):
        ...
        zoom_trans_gt = mx.sym.Custom(
            zoom_factor=zoom_factor, trans_delta=trans_gt, name="ZoomTrans", op_type="ZoomTrans", b_inv_zoom=False
        )
        ...
        pred_iter = self.get_loss(  # noqa:F841
            cfg, small_cfg, conv_feat, deconv_feat, labels_dict, group_list, regressor_idx=0
        )
    ...
    def get_test_symbol_share(self, cfg):
        ...
        if cfg.network.PRED_MASK and (cfg.TEST.UPDATE_MASK not in ["init", "box_rendered"] or not cfg.TEST.FAST_TEST):
            ...
            mask_pred_resize_crop = mx.symbol.Crop(
                *[mask_pred_resize, image_observed], offset=(8, 8), name="mask_pred_resize_crop"
            )
            zoom_mask_observed_pred = mx.sym.Activation(
                data=mask_pred_resize_crop, act_type="sigmoid", name="zoom_mask_observed_prob_iter"
            )
        ...
        if cfg.network.PRED_FLOW and not cfg.TEST.FAST_TEST:
            ...
            zoom_flow_est_crop = mx.symbol.Crop(
                *[flow_est_resize, image_observed], offset=(8, 8), name="zoom_flow_est_crop"
            )
        ...
        rot_est = mx.symbol.FullyConnected(name="rot", data=conv_feat, num_hidden=rot_param, no_bias=False)
        zoom_trans_est = mx.symbol.FullyConnected(name="trans", data=conv_feat, num_hidden=3, no_bias=False)
        trans_est = mx.sym.Custom(
            zoom_factor=zoom_factor,
            trans_delta=zoom_trans_est,
        ...
        if cfg.network.pretrained == "xavier":
            ...
            if cfg.network.init_from_flownet:
                ...
                if cfg.network.ROT_TYPE == "EULER":
                    ...
                    rot_weight = (
                        np.random.rand(self.arg_shape_dict["rot_weight"][0], self.arg_shape_dict["rot_weight"][1])
                        * 0.01
                    )
            ...
            for k in self.arg_shape_dict:
                if (k not in arg_params) and (k.endswith("weight") or k.endswith("bias")):
