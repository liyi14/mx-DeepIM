...
class deepIM_flownet(Symbol):
    ...
    def get_convs(self, data_iter, big_cfg, small_cfg, iter_idx):
        ...
        if big_cfg.network.PRED_FLOW or big_cfg.network.PRED_MASK:
            ...
            deconv5 = mx.symbol.Deconvolution(
                ...
                kernel=(4, 4),
                stride=(2, 2),
                no_bias=False)  # (B,512, 18, 22)
            crop_deconv5 = mx.symbol.Crop(
                name='crop_deconv5', *[deconv5, ReLU8],
                offset=(1, 1))  # (B,512,15,20)
            ...
            Concat2 = mx.symbol.Concat(
                name='Concat2',
                *[ReLU8, ReLU11, crop_upsampled_flow6_to_5])  # (B, 1026,15,20)
            ...
            crop_upsampled_flow5_to_4 = mx.symbol.Crop(
                ...
                *[upsample_flow5to4, ReLU6],
                offset=(1, 1))  # (B,2,32,42)
            Concat3 = mx.symbol.Concat(
                name='Concat3',
                *[ReLU6, ReLU12, crop_upsampled_flow5_to_4])  # (B,770,30,40)
            feat_list[1] = Concat3

        return feat_list

    def get_loss(self, big_cfg, small_cfg, conv_feat, deconv_feat, labels,
                 group_list, regressor_idx):
        pred = {}
        ...
        if big_cfg.network.PRED_FLOW:
            ...
            flow_loss_ = labels['flow_weights'] * mx.sym.square(
                data=(flow_est_crop -
                      labels['flow'] / big_cfg.dataset.NORMALIZE_FLOW),
                name='flow_loss')
            ...
            group_list.append(
                mx.sym.BlockGrad(
                    flow_est_crop * big_cfg.dataset.NORMALIZE_FLOW))
        ...
        rot_est_norm = mx.sym.L2Normalization(
            data=rot_est, name='normalize_quat')
        ...
        if small_cfg.SE3_DIST_LOSS:
            ...
            rot_gt_dot = mx.sym.expand_dims(
                rot_gt, axis=1, name='expand_rot_gt')
            rot_loss_ = 1 - mx.sym.square(
                mx.sym.batch_dot(
                    rot_gt_dot, rot_est_norm_dot, transpose_b=True),
                name='rot_loss')
        ...
        if small_cfg.SE3_PM_LOSS:
            ...
            if small_cfg.SE3_PM_LOSS_TYPE == 'L1':
                point_matching_loss_all = mx.sym.abs(
                    data=(point_cloud_observed_est - point_cloud_observed) /
                    norm_term,
                ...
                raise Exception('Unknown Point Matching Loss Type: {}'.format(
        ...
        if big_cfg.network.PRED_MASK:
            ...
            group_list.append(
                mx.sym.BlockGrad(
                    unzoomed_mask_pred, name='unzoomed_mask_pred'))
    ...
    def get_train_symbol(self, cfg):
        ...
        if small_cfg.SE3_PM_LOSS:
            ...
            point_cloud_weights = mx.symbol.Variable(
                name='point_cloud_weights')
            point_cloud_observed = mx.symbol.Variable(
                name='point_cloud_observed')
        ...
        if cfg.TRAIN.VISUALIZE:
            group_list.append(
                mx.sym.BlockGrad(zoom_mask_gt_observed))  # zoomed
    ...
    def get_test_symbol_share(self, cfg):
        ...
        conv_feat, deconv_feat = self.get_convs(data_iter, cfg, cfg.train_iter,
                                                0)
        if cfg.network.PRED_MASK and (
            ...
            group_list.append(
                mx.sym.BlockGrad(
                    mask_observed_pred, name='mask_observed_pred'))

            if not cfg.TEST.FAST_TEST:
                group_list.append(zoom_mask_observed_pred)
                group_list.append(
                    mx.sym.BlockGrad(
                        zoom_mask_observed, name='zoom_mask_observed'))
                group_list.append(
                    mx.sym.BlockGrad(
                        zoom_mask_observed_pred,
                        name='zoom_mask_observed_pred'))
                group_list.append(
                    mx.sym.BlockGrad(
                        zoom_image_observed, name='zoom_image_observed'))
                group_list.append(
        ...
        if cfg.network.pretrained == 'xavier':
            ...
            for param_name in arg_params:
                if param_name.endwith('bias'):
                    ...
                    init_xavier._init_weight(param_name,
                                             arg_params[param_name])
        else:
            ...
            if cfg.network.init_from_flownet:
                ...
                rot_bias_init = mx.nd.zeros(
                    shape=self.arg_shape_dict['rot_bias'])
                ...
                init_xavier = mx.init.Xavier()
                init_xavier._init_weight('fc6_weight',
                                         arg_params['fc6_weight'])
                init_xavier._init_weight('fc7_weight',
                                         arg_params['fc7_weight'])

                if cfg.network.PRED_FLOW:
                    init = mx.init.Initializer()
                    arg_params['upsampling_weight'] = mx.nd.zeros(
                        shape=self.arg_shape_dict['upsampling_weight'])
                    init._init_bilinear('upsample_weight',
                                        arg_params['upsampling_weight'])
                ...
                if cfg.network.PRED_MASK:
                    ...
                    init._init_bilinear('mask_upsampling_weight',
                                        arg_params['mask_upsampling_weight'])

                print('arg_shape_dict: ', self.arg_shape_dict.keys())

            # init from pre-trained
            for k in self.arg_shape_dict:
                if (k not in arg_params) and (k.endswith('weight')
                                              or k.endswith('bias')):
                    ...
                    if not cfg.network.REGRESSOR_NUM == 1:
                        ...
                        elif k.startswith('trans_iter') and k.endswith(
                                'weight'):
