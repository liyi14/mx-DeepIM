...
if __name__ == '__main__':
    ...
    proj2d = mx.sym.Custom(mask_real_est=mask_real_est, mask_real_gt=mask_real_gt, mask_rendered=mask_rendered,
                           src_pose=src_pose, K=K.flatten(),
    ...
    for idx in range(batch_size):
        ...
        # mask_real_gt
        tmp = cv2.imread(
            os.path.join(cur_dir,   '../../data/render_v5/data/render_real/002_master_chef_can/0012/{:06}-depth.png'.format(
                sub_idx1[idx])),
                                         cv2.IMREAD_UNCHANGED)/10000.
        tmp[tmp!=0] = 1
        tmp = tmp[np.newaxis, :, :]
    ...
    exe1 = proj2d.simple_bind(ctx=ctx, mask_real_est=v_mask_real_est.shape,
                              mask_real_gt=v_mask_real_gt.shape, mask_rendered=v_mask_rendered.shape,
                              src_pose=v_src_pose.shape)

    # forward
    def simple_forward(exe1, v_mask_real_est, v_mask_real_gt, v_mask_rendered, v_src_pose,
                       ctx, is_train=False):
