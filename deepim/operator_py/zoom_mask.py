...
class ZoomMaskOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        mask_rendered_np[
            mask_rendered_np > 0.2] = 1  # if the mask_rendered input is depth
        mask_rendered_np[
            mask_rendered_np <= 0.2] = 0  # if the mask_rendered input is depth
        ...
        for batch_idx in range(batch_size):
            ...
            affine_matrix = mx.nd.array([[wx, 0, tx], [0, wy, ty]],
                                        ctx=ctx).reshape((1, 6))
...
class ZoomMaskProp(mx.operator.CustomOpProp):
    ...
    def list_arguments(self):
        input_list = [
            'mask_observed', 'mask_gt_observed', 'mask_rendered', 'src_pose'
        ]
        return input_list

    def list_outputs(self):
        output_list = [
            'zoom_mask_observed', 'zoom_mask_gt_observed',
            'zoom_mask_rendered', 'zoom_factor'
        ]
        return output_list
