...
class ZoomMaskWithFactorOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for batch_idx in range(batch_size):
            ...
            a = mx.ndarray.GridGenerator(data=affine_matrix, transform_type='affine',
...
if __name__ == '__main__':
    ...
    proj2d_1 = mx.sym.Custom(mask_real_est=mask_real_est, mask_real_gt=mask_real_gt, mask_rendered=mask_rendered,
                           src_pose=src_pose,
                           name='updater', op_type='ZoomMask',
                           K=K.flatten(),
    ...
    for idx in range(batch_size):
        ...
        v_src_pose[idx] = np.loadtxt(os.path.join(cur_dir,
                          '../../data/render_v5/data/render_real/002_master_chef_can/0012/{:06}-pose.txt'.format(
                              sub_idx2[idx])), skiprows=1)
    print('bind1')
    exe1 = proj2d_1.simple_bind(ctx=ctx, mask_real_est=v_mask_real_est.shape,
                              mask_real_gt=v_mask_real_gt.shape, mask_rendered=v_mask_rendered.shape,
    ...
    proj2d_2 = mx.sym.Custom(zoom_factor=zoom_factor_sym, mask=mask_sym,
                           name='updater1', op_type='ZoomMaskWithFactor',
                           height=height, width=width, b_inv_zoom=True)
