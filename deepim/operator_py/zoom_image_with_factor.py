...
class ZoomImageWithFactorOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self.center_map is None:
            ...
            self.center_map[:, 0, spot_start_y:spot_end_y, spot_start_x:spot_end_x] = 255.0# red center
        ...
        zoom_image_rendered_array = mx.nd.BilinearSampler(image_rendered_array, grid_array)
        zoom_image_real_array = mx.nd.BilinearSampler(image_real_array, grid_array)
        if self.high_light_center:
            zoom_image_rendered_array = mx.ndarray.maximum(zoom_image_rendered_array, self.center_map)
...
class ZoomImageWithFactorProp(mx.operator.CustomOpProp):
    def __init__(self, width=640, height=480, pixel_means='[0 0 0]', high_light_center='False'):
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return ZoomImageWithFactorOperator(self.height, self.width, self.pixel_means,
                                           self.hight_light_center)
...
if __name__ == '__main__':
    ...
    proj2d = mx.sym.Custom(zoom_factor=zoom_factor, image_real=image_real, image_rendered=image_rendered,
                           pixel_means=np.array([pixel_means, pixel_means, pixel_means]).flatten(),
                           name='updater', op_type='ZoomImageWithFactor',
                           height=height, width=width, high_light_center=True)
    ...
    for idx in range(batch_size):
        v_image_rendered[idx] = cv2.imread(
            os.path.join(cur_dir, '../../data/render_v5/data/rendered/0003/{}_{}-color.png'.format(img_idx[idx], sub_idx1[idx])),
            cv2.IMREAD_COLOR).transpose([2, 0, 1])
        v_image_real[idx] = cv2.imread(
            os.path.join(cur_dir,'../../data/render_v5/data/rendered/0003/{}_{}-color.png'.format(img_idx[idx], sub_idx2[idx])),
            cv2.IMREAD_COLOR).transpose([2, 0, 1])

    exe1 = proj2d.simple_bind(ctx=ctx, zoom_factor=v_zoom_factor.shape,
                              image_real=v_image_real.shape, image_rendered=v_image_rendered.shape)
    ...
    exe1.arg_dict['image_rendered'][:] = mx.nd.array(v_image_rendered - pixel_means, ctx=ctx)
    import time
