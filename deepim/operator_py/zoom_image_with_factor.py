...
class ZoomImageWithFactorOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self.center_map is None:
            self.center_map = np.zeros(
                [batch_size, 3, self.height, self.width])
            spot_start_x = int(
                np.floor(float(self.width) / 2 - self.spot_radius))
            spot_end_x = int(np.ceil(float(self.width) / 2 + self.spot_radius))
            spot_start_y = int(
                np.floor(float(self.height) / 2 - self.spot_radius))
            spot_end_y = int(
                np.ceil(float(self.height) / 2 + self.spot_radius))
            self.center_map[:, 0, spot_start_y:spot_end_y, spot_start_x:
                            spot_end_x] = 255.0  # red center
        ...
        zoom_image_real_array = mx.nd.BilinearSampler(image_real_array,
                                                      grid_array)
...
if __name__ == '__main__':
    ...
    exe1.arg_dict['image_real'][:] = mx.nd.array(
