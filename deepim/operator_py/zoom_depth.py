...
class ZoomDepthOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        zoom_depth_real_array = mx.ndarray.BilinearSampler(
            depth_real_array, grid_array)
...
if __name__ == '__main__':
    ...
    proj2d = mx.sym.Custom(
        ...
        height=height,
        width=width)
    v_zoom_factor = np.ones(
        (batch_size, 4)) * 2  # np.random.rand(batch_size, 4) * 2
    ...
    for idx in range(batch_size):
        v_depth_rendered[idx] = cv2.imread(
            '/home/yili/PoseEst/mx-DeepPose/data/render_v5/data/rendered/0003/{}_{}-depth.png'
            .format(img_idx[idx], sub_idx1[idx]), cv2.IMREAD_COLOR).transpose(
                [2, 0, 1])[np.newaxis, 0, :, :] / 10000.0
        v_depth_real[idx] = cv2.imread(
            '/home/yili/PoseEst/mx-DeepPose/data/render_v5/data/rendered/0003/{}_{}-depth.png'
            .format(img_idx[idx], sub_idx2[idx]), cv2.IMREAD_COLOR).transpose(
                [2, 0, 1])[np.newaxis, 0, :, :] / 10000.0
    ...
    exe1.arg_dict['depth_rendered'][:] = mx.ndarray.array(
    ...
    for batch_idx in range(batch_size):
        ...
        d_rendered = np.squeeze(v_depth_rendered[batch_idx].transpose(
            [1, 2, 0]))
        z_d_real = np.squeeze(zoom_depth_real[batch_idx].transpose([1, 2, 0]))
        z_d_rendered = np.squeeze(zoom_depth_rendered[batch_idx].transpose(
            [1, 2, 0]))
        fig = plt.figure()
