...
class transform3dOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for i in range(batch_size):
            Rm_tgt[i] = mx.nd.array(R_transform(Rm_src[i].asnumpy(), Rm_delta[i].asnumpy(),
                                                rot_coord=self.rot_coord), ctx=ctx)

        # T_delta, T_src --> T_tgt
        T_tgt = mx.nd.zeros((batch_size, 3), ctx=ctx)
        for i in range(batch_size):
            if self.rot_coord.lower() == "naive":
                T_tgt[i] = mx.nd.array(T_transform_naive(Rm_delta[i].asnumpy(), T_src[i].asnumpy(), T_delta[i].asnumpy()))
            else:
                T_tgt[i] = mx.nd.array(T_transform(T_src[i].asnumpy(), T_delta[i].asnumpy(), self.T_means, self.T_stds,
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        if self.rot_coord.lower() == "naive":
            ...
            for i in range(batch_size):
                T_delta_diff[i] = self.T_transform_backward(T_tgt_diff[i], T_src[i], T_delta[i], T_means, T_stds,
                                                            ctx=ctx, rot_coord=self.rot_coord)
        ...
        for batch_idx in range(batch_size):
            ...
            R_delta_diff[batch_idx] = self.quat2mat_backward(Rm_delta_diff[batch_idx], rotation[batch_idx],
    ...
    def quat2mat_forward(self, q):
        ...
        return mx.nd.array(
            [[1.0 - (yY + zZ), xY - wZ, xZ + wY],
             ...
             [xZ - wY, yZ + wX, 1.0 - (xX + yY)]], ctx=q.context, dtype='float32')
...
if __name__ == '__main__':
    ...
    tran3d = mx.sym.Custom(point_cloud=point_cloud, rotation=rotation, translation=translation, pose_src=pose_src,
                           name='tran3d', op_type='Transform3D', T_means=T_means, T_stds=T_stds, rot_coord=rot_coord)
    ...
    exe1 = tran3d.simple_bind(ctx=ctx, point_cloud=v_point_cloud.shape,
                              rotation=v_rotation.shape, translation=v_translation.shape,
    ...
    for iter in range(batch_size):
        ...
        pc_est = y1[iter]
        assert (np.abs(pc_est-pc_gt).max(1) < 1e-4).all(), "forward check failed, {} > {}".format(np.abs(pc_est-pc_gt).sum(), thresh)
    ...
    for test_idx in range(int(v_translation.size/2)):
        ...
        if not (abs_diff < thresh or rel_diff < thresh):
            print("test_idx:{}, translation backward error, abs_diff:{}, rel_diff:{}, thresh:{}".format(
                test_idx, abs_diff, rel_diff, thresh))

        if (abs_diff < thresh or rel_diff < thresh):
            print("trans backward check succeed")
        else:
            print("sample_point: {}, grad_est: {}, grad_gt: {}, abs_diff: {}, rel_diff: {}, v_translation: {}"
    ...
    for i in range(v_rotation.shape[0]):
        for j in range(v_rotation.shape[1]):
            ...
            if (abs_diff < thresh or rel_diff < thresh):
                ...
                print("sample_point: {}, grad_est: {}, grad_gt: {}, abs_diff: {}, rel_diff: {}, v_rotation: {}" \
                        .format((i, j), grad_est, grad_gt, grad_est - grad_gt,
                                np.abs(grad_est - grad_gt) / max(np.abs(grad_gt), 1e-15),
                                v_rotation[i]))
