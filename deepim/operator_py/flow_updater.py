...
class flowUpdaterOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self.base_coord is None:
            ...
            self.w_ori = np.tile(
                w_ori.reshape((1, 1, -1)), (self.batch_size, 1, 1))
            self.h_ori = np.tile(
                h_ori.reshape((1, 1, -1)), (self.batch_size, 1, 1))
        ...
        batch_ones = mx.nd.ones((batch_size, 1, self.width * self.height),
                                ctx=ctx)
        ...
        z_proj = z_proj + 1E-15
        pw = mx.nd.minimum(
            mx.nd.maximum(mx.nd.round(w_proj / z_proj), 0),
            self.width - 1).asnumpy().astype(np.int)
        ph = mx.nd.minimum(
            mx.nd.maximum(mx.nd.round(h_proj / z_proj), 0),
            self.height - 1).asnumpy().astype(np.int)
        pz = z_proj.asnumpy()
        valid_in_src = (depth_src.reshape(
            (self.batch_size, 1, self.height * self.width))).asnumpy() > 1E-10
        ...
        valid_points = valid_points.reshape((self.batch_size, 1, self.height,
                                             self.width))
...
class flowUpdaterProp(mx.operator.CustomOpProp):
    def __init__(self, K, thresh, batch_size, height, width, wh_rep='False'):
        ...
        self.K = np.fromstring(
            K[1:-1], dtype=np.float32, sep=' ').reshape((3, 3))
    ...
    def infer_shape(self, in_shape):
        ...
        input_shape = [
            depth_src_shape, depth_tgt_shape, pose_src_shape, pose_tgt_shape
        ]
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return flowUpdaterOperator(self.K, self.thresh, self.batch_size,
                                   self.height, self.width, self.wh_rep)
...
if __name__ == '__main__':
    ...
    for i in range(batch_size):
        v_depth_rendered[i, 0, :, :] = cv2.imread(
            depth_path.format(src_img_idx[i]),
            cv2.IMREAD_UNCHANGED) / DEPTH_FACTOR
        v_depth_real[i, 0, :, :] = cv2.imread(
            depth_path.format(tgt_img_idx[i]),
            cv2.IMREAD_UNCHANGED) / DEPTH_FACTOR
    ...
    # forward
    for i in range(5):
        exe1.arg_dict['depth_rendered'][:] = mx.nd.array(
            v_depth_rendered, ctx=ctx)
        ...
        for j in range(batch_size):
            ...
            img_real = cv2.imread(
                color_path.format(tgt_img_idx[j]), cv2.IMREAD_COLOR)
            ...
            for h in range(height):
                for w in range(width):
                    if flow_weights[h, w]:
                        ...
                        img_tgt = cv2.line(
                            img_tgt, (np.round(w + cur_flow[1]).astype(int),
            ...
            depth_rendered = cv2.imread(
                depth_path.format(src_img_idx[j]),
                cv2.IMREAD_UNCHANGED) / DEPTH_FACTOR
