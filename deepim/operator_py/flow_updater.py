...
class flowUpdaterOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if self.base_coord is None:
            ...
            w_ori, h_ori = np.meshgrid(np.linspace(0, self.width - 1, self.width),
        ...
        X = mx.nd.broadcast_mul(depth_src.reshape((self.batch_size, 1, self.width*self.height)),
                                     self.batch_R)
...
class flowUpdaterProp(mx.operator.CustomOpProp):
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return flowUpdaterOperator(self.K, self.thresh, self.batch_size, self.height, self.width,
                                   self.wh_rep)
...
if __name__ == '__main__':
    ...
    proj2d = mx.sym.Custom(depth_src=depth_rendered, depth_tgt=depth_real,
                           pose_src=pose_src, pose_tgt=pose_tgt,
                           K=K.flatten(),
                           name='updater', op_type='FlowUpdater', batch_size=batch_size,
                           height=height, width=width, thresh=flow_thresh, wh_rep=wh_rep)
    ...
    # bind
    exe1 = proj2d.simple_bind(ctx=ctx, depth_rendered=v_depth_rendered.shape,
                              depth_real=v_depth_real.shape,
                              pose_src=v_pose_src.shape, pose_tgt=v_pose_tgt.shape)
    ...
    for i in range(5):
        ...
        for j in range(batch_size):
            img_rendered = cv2.imread(color_path.format(src_img_idx[j]),
                                  cv2.IMREAD_COLOR)
            img_rendered = img_rendered[:, :, [2,1,0]]
            img_real = cv2.imread(color_path.format(tgt_img_idx[j]),
                                  cv2.IMREAD_COLOR)
            ...
            depth_rendered = cv2.imread(depth_path.format(src_img_idx[j]),
                                      cv2.IMREAD_UNCHANGED) / DEPTH_FACTOR
