...
if __name__ == '__main__':
    ...
    proj2d = mx.sym.Custom(image_real=image_real, image_rendered=image_rendered, src_pose=src_pose, K=K.flatten(),
                           pixel_means=np.array([pixel_means, pixel_means, pixel_means]).flatten(),
                           name='updater', op_type='ZoomImage',
    ...
    for idx in range(batch_size):
        v_image_rendered[idx] = cv2.imread(
            os.path.join(cur_dir, '../../data/render_v5/data/rendered/0003/{}_{}-color.png'
        ...
        v_src_pose[idx] = np.loadtxt(
            os.path.join(cur_dir, '../../data/render_v5/data/rendered/0003/{}_{}-pose.txt'
                .format(img_idx[idx], sub_idx1[idx])),
            skiprows=1)
    exe1 = proj2d.simple_bind(ctx=ctx, image_real=v_image_real.shape, image_rendered=v_image_rendered.shape, src_pose=v_src_pose.shape)
