...
class ZoomImageOperator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        for batch_idx in range(batch_size):
            ...
            left_dist = max(zoom_c_x - obj_rendered_start_x,
                            zoom_c_x - obj_real_start_x)
            right_dist = max(obj_rendered_end_x - zoom_c_x,
                             obj_real_end_x - zoom_c_x)
            up_dist = max(zoom_c_y - obj_rendered_start_y,
                          zoom_c_y - obj_real_start_y)
            down_dist = max(obj_real_end_y - zoom_c_y,
                            obj_rendered_end_y - zoom_c_y)
            crop_height = np.max([
                0.75 * right_dist, 0.75 * left_dist, up_dist, down_dist
            ]) * 1.4 * 2
        ...
        zoom_image_real_array = mx.ndarray.BilinearSampler(
            image_real_array, grid_array)
...
class ZoomImageProp(mx.operator.CustomOpProp):
    def __init__(self, K, width=640, height=480, pixel_means='[0 0 0]'):
        ...
        self.pixel_means = np.fromstring(
            pixel_means[1:-1], dtype=np.float32, sep=' ').reshape(3)
        self.pixel_means = self.pixel_means[::-1]
        self.pixel_means = self.pixel_means[np.newaxis, :, np.newaxis, np.
                                            newaxis]
    ...
    def create_operator(self, ctx, shapes, dtypes):
        return ZoomImageOperator(self.K, self.height, self.width,
                                 self.pixel_means)
...
if __name__ == '__main__':
    ...
    proj2d = mx.sym.Custom(
        ...
        src_pose=src_pose,
        K=K.flatten(),
        pixel_means=np.array([pixel_means, pixel_means,
                              pixel_means]).flatten(),
        name='updater',
        op_type='ZoomImage',
        height=height,
    ...
    for idx in range(batch_size):
        v_image_rendered[idx] = cv2.imread(
            os.path.join(
                cur_dir,
                '../../data/render_v5/data/rendered/0003/{}_{}-color.png'.
                format(img_idx[idx], sub_idx1[idx])),
            cv2.IMREAD_COLOR).transpose([2, 0, 1])
        v_image_real[idx] = cv2.imread(
            os.path.join(
                cur_dir,
                '../../data/render_v5/data/rendered/0003/{}_{}-color.png'.
                format(img_idx[idx], sub_idx2[idx])),
            cv2.IMREAD_COLOR).transpose([2, 0, 1])
        v_src_pose[idx] = np.loadtxt(
            os.path.join(
                cur_dir,
                '../../data/render_v5/data/rendered/0003/{}_{}-pose.txt'.
                format(img_idx[idx], sub_idx1[idx])),
            skiprows=1)
    ...
    for batch_idx in range(batch_size):
        ...
        im_rendered = v_image_rendered[batch_idx].transpose([1, 2, 0]).astype(
            np.uint8)
        z_im_real = zoom_image_real[batch_idx].transpose([1, 2, 0
                                                          ]) + pixel_means
        z_im_rendered = zoom_image_rendered[batch_idx].transpose(
            [1, 2, 0]) + pixel_means
        fig = plt.figure()
