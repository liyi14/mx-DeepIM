...
def parse_args():
    ...
    parser.add_argument(
        '--temp', help='turn on visualization', action='store_true')
    parser.add_argument(
...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr,
    ...
    if not args.temp:
        ...
        dst_file = os.path.join(
            final_output_path, '{}_{}.py'.format(
                config.symbol, time.strftime('%Y-%m-%d-%H-%M')))
    ...
    if config.network.PRED_FLOW:
        ...
        max_label_shape.append(('flow_weights', (config.TRAIN.BATCH_PAIRS, 2,
                                                 max_scale[0], max_scale[1])))
    if config.train_iter.SE3_PM_LOSS:
        max_label_shape.append(('point_cloud_model',
                                (config.TRAIN.BATCH_PAIRS, 3,
                                 config.train_iter.NUM_3D_SAMPLE)))
        max_label_shape.append(('point_cloud_weights',
                                (config.TRAIN.BATCH_PAIRS, 3,
                                 config.train_iter.NUM_3D_SAMPLE)))
        max_label_shape.append(('point_cloud_observed',
                                (config.TRAIN.BATCH_PAIRS, 3,
                                 config.train_iter.NUM_3D_SAMPLE)))
    if config.network.PRED_MASK:
        max_label_shape.append(('mask_gt_observed',
                                (config.TRAIN.BATCH_PAIRS, 1, max_scale[0],
                                 max_scale[1])))
...
def main():
    ...
    train_net(args, ctx, config.network.pretrained,
              config.network.pretrained_epoch, config.TRAIN.model_prefix,
