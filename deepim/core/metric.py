...
class MaskLossMetric(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        mask_loss = -(mask_gt * np.log(mask_prob + 1e-19) +
...
class SimpleVisualize(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        zoom_mask_real_gt = np.squeeze(
            preds[-5].asnumpy()[sel_img_idx, 0, :, :])
        zoom_mask_real_est = np.squeeze(
            preds[-4].asnumpy()[sel_img_idx, 0, :, :])
        zoom_mask_rendered = np.squeeze(
            preds[-3].asnumpy()[sel_img_idx, 0, :, :])
        ...
        for iter_idx in range(1):
            print('rot_est_iter_{}: {}, \nrot_gt_iter_{} : {}'.format(
                iter_idx,
                preds[self.pred.index('rot_est')].asnumpy()[sel_img_idx],
                iter_idx,
                preds[self.pred.index('rot_gt')].asnumpy()[sel_img_idx]))
            print('trans_est_iter_{}: {}, \ntrans_gt_iter_{} : {}'.format(
                iter_idx,
                preds[self.pred.index('trans_est')].asnumpy()[sel_img_idx],
                iter_idx,
                preds[self.pred.index('trans_gt')].asnumpy()[sel_img_idx]))
...
class MaskVisualize(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
                ...
                for w in range(0, width, 3):
                    if visible[h, w]:
                        ...
                        mesh_rendered = cv2.circle(
                            ...
                            1, (h * 255 / height, 255 - w * 255 / width,
                                w * 255 / width), 5)

                        mesh_real = cv2.circle(
                            mesh_real, (np.round(w + cur_flow[1]).astype(int),
        ...
        self.save_fig(image_real, '{}/{}_image_real.png'.format(
            self.save_dir, time_str))

        self.save_fig(
            image_rendered, '{}/{}_image_rendered.png'.format(
                self.save_dir, time_str))

        self.save_fig(
            zoom_image_real, '{}/{}_zoom_image_real.png'.format(
                self.save_dir, time_str))

        self.save_fig(
            zoom_image_rendered, '{}/{}_zoom_image_rendered.png'.format(
                self.save_dir, time_str))

        if self.cfg.network.PRED_MASK:
            self.save_fig(
                zoom_mask_real_est, '{}/{}_zoom_mask_real_est.png'.format(
                    self.save_dir, time_str))

            self.save_fig(
                zoom_mask_real_gt, '{}/{}_zoom_mask_real_gt.png'.format(
                    self.save_dir, time_str))

            self.save_fig(
                zoom_mask_rendered, '{}/{}_zoom_mask_rendered.png'.format(
                    self.save_dir, time_str))

            self.save_fig(
                zoom_mask_pred_bin, '{}/{}_zoom_mask_pred_bin.png'.format(
                    self.save_dir, time_str))

        if self.cfg.network.PRED_FLOW:
            self.save_fig(
                mesh_real, '{}/{}_mesh_real.png'.format(
                    self.save_dir, time_str))

            self.save_fig(
                mesh_rendered, '{}/{}_mesh_rendered.png'.format(
                    self.save_dir, time_str))

            self.save_fig(
                mesh_real_est, '{}/{}_mesh_real_est.png'.format(
                    self.save_dir, time_str))
        print('=====================')
...
class MinibatchVisualize(mx.metric.EvalMetric):
    ...
    def update(self, labels, preds):
        ...
        print('image_rendered: ', image_rendered.shape, image_rendered.min(),
              image_rendered.max())
        ...
        for h in range(0, height, 3):
            for w in range(0, width, 3):
                if visible[h, w]:
                    ...
                    point = np.round(
                        [w + cur_flow_est[1], h + cur_flow_est[0]]).astype(int)
                    point[0] = min(max(point[0], 0), width)
                    point[1] = min(max(point[1], 0), height)
                    mesh_real_est = cv2.circle(
                        mesh_real_est, (point[0], point[1]), 1,
