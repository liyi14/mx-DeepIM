...
def pred_eval(config,
    ...
    if (config.TEST.test_iter > 1 or config.TEST.VISUALIZE) and True:
        ...
        if config.dataset.dataset.startswith('ModelNet'):
            ...
            render_machine = Render_Py(
                config.dataset.model_dir, config.dataset.class_name, K, width,
                height, config.dataset.ZNEAR, config.dataset.ZFAR)
    ...
    if config.TEST.PRECOMPUTED_ICP:
        ...
        print("rot = {} +/- {}".format(
            np.mean(all_rot_err[class_id][0]),
            np.std(all_rot_err[class_id][0])))
        print("trans = {} +/- {}".format(
            np.mean(all_trans_err[class_id][0]),
    ...
    for idx, data_batch in enumerate(test_data):
        ...
        for output in output_all:
            ...
            if config.network.PRED_MASK and config.TEST.UPDATE_MASK not in [
                ...
                mask_pred = np.squeeze(output['mask_observed_pred_output'].
        ...
        for batch_idx in range(0, test_data.batch_size):
            ...
            for pose_iter_idx in range(config.TEST.test_iter):
                ...
                pose_rendered_update = RT_transform(
                    pose_rendered, rst_iter[0]['se3'][:-3],
                    rst_iter[0]['se3'][-3:], config.dataset.trans_means,
                    config.dataset.trans_stds, config.network.ROT_COORD)
                ...
                if pose_iter_idx < (
                        config.TEST.test_iter - 1) or config.TEST.VISUALIZE:
                    t = time.time()
                    # get refined image
...
def get_onclick(ax_real_list, ax_rendered_list, data, fig):
    def onclick(event):
        ...
        cur_flow_gt = flow_i2r[point_rendered[1],
                               point_rendered[0], :].flatten()
        ...
        cur_flow_est = flow_i2r_est[point_rendered[1],
                                    point_rendered[0], :].flatten()
        if not config.network.STANDARD_FLOW_REP:
