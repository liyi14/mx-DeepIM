...
class ModelNet_v1(IMDB):
    def __init__(self, cfg, image_set, root_path, devkit_path, class_name, result_path=None, mask_size=-1, binary_thresh=None, mask_syn_name=''):
        ...
        if len(mask_syn_name)>0:
            super(ModelNet_v1, self).__init__('ModelNet_render_v1_'+mask_syn_name, image_set, root_path, devkit_path, result_path)  # set self.name
        else:
            super(ModelNet_v1, self).__init__('ModelNet_render_v1', image_set, root_path, devkit_path, result_path)  # set self.name
    ...
    def _load_object_points_diameters(self):
        ...
        for cls_idx, cls_name in enumerate(self.classes):
            ...
            if len(base_cns) == 3:
                obj_file = os.path.join(self._modelnet_path, 'ModelNet40', base_cns[0] + '_' + base_cns[1],
                                    set, '{}.obj'.format(base_cls_name))
            elif len(base_cns) == 2:
                obj_file = os.path.join(self._modelnet_path, 'ModelNet40', base_cns[0],
                                        set, '{}.obj'.format(base_cls_name))
            else:
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        if check:
            assert os.path.exists(depth_file), 'Path does not exist: {}'.format(depth_file)
        return depth_file
