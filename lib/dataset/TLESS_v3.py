...
class TLESS_v3(IMDB):
    def __init__(self,
        ...
        if len(mask_syn_name) > 0:
            super(TLESS_v3, self).__init__('TLESS_render_v3_' + mask_syn_name,
                                           image_set, root_path, devkit_path,
                                           result_path)  # set self.name
        else:
            super(TLESS_v3,
                  self).__init__('TLESS_render_v3', image_set, root_path,
                                 devkit_path, result_path)  # set self.name
        ...
        self._tless_path = self._get_default_path()

        if image_set.startswith('yu_val_v02'):
            self.rendered_data_path = os.path.join(devkit_path, 'data',
                                                   'rendered_Yu_v02')
        elif image_set.startswith('yu_val'):
            self.rendered_data_path = os.path.join(devkit_path, 'data',
                                                   'rendered_Yu_v01')
        elif image_set.startswith('train_v1') or image_set.startswith(
                'my_val_v1') or image_set.startswith('my_minival_v1'):
            self.rendered_data_path = os.path.join(devkit_path, 'data',
                                                   'rendered_v1')
        elif image_set.startswith('train') or image_set.startswith(
                'my_val') or image_set.startswith('my_minival'):
            self.rendered_data_path = os.path.join(devkit_path, 'data',
                                                   'rendered')
        else:
            raise Exception("unknown prefix of " + image_set)

        print('TLESS_v3 rendered_data_path: {}'.format(
            self.rendered_data_path))

        self.mask_syn_path = ''
        if image_set.startswith('train'):
            self.phase = 'train'
            if len(mask_syn_name) > 0:
                self.mask_syn_path = os.path.join(devkit_path, 'data',
                                                  mask_syn_name)
        elif image_set.startswith('my_val') or image_set.startswith(
                'yu_val') or image_set.startswith('my_minival'):
            self.phase = 'val'
        else:
            raise Exception("unknown prefix of " + image_set)
        ...
        if image_set.startswith('yu_val_v02'):
            if os.path.exists(
                    os.path.join(devkit_path, 'data', 'mask_Yu_v02')):
                self.mask_est_path = os.path.join(devkit_path, 'data',
                                                  'mask_Yu_v02')
            else:
        ...
        self.classes = ['{:02d}'.format(i) for i in range(1, 31)]  # 01 --> 30
    ...
    def _get_default_path(self):
        """
        Return the default path where TLESS is expected to be installed.
        """
        ROOT_DIR = os.path.join(os.path.dirname(__file__), '..', '..')
        return os.path.join(ROOT_DIR, 'data', 'TLESS')
    ...
    def _load_object_points(self):
        ...
        for cls_idx, cls_name in enumerate(self.classes):
            point_file = os.path.join(self._tless_path, 'models', cls_name,
                                      'points.xyz')
    ...
    def _load_object_diameter(self):
        diameters = {}
        models_info_file = os.path.join(self._tless_path, 'models',
                                        'models_info.yml')
        assert os.path.exists(
            models_info_file), 'Path does not exist: {}'.format(
                models_info_file)
        with open(models_info_file, 'r') as f:
            m_dict = yaml.load(f)
            for cls_idx, cls_name in enumerate(self.classes):
                diameter = m_dict[int(cls_name)]['diameter']
                diameters[cls_name] = diameter / 1000.  # mm --> m
        return diameters
    ...
    def load_image_set_index(self):
        ...
        :return:
        """
        image_set_index_file = os.path.join(self.devkit_path, 'image_set',
                                            self.image_set + '.txt')
        assert os.path.exists(
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        """
        if type == 'real':
            image_file = os.path.join(self.real_data_path,
                                      index + '-color.png')
        elif type == 'render_real':
            image_file = os.path.join(self.render_real_data_path,
                                      index + '-color.png')
        elif type == 'rendered':
            image_file = os.path.join(self.rendered_data_path,
                                      index + '-color.png')
        if check:
            assert os.path.exists(
                image_file), 'type: {}, Path does not exist: {}'.format(
                    type, image_file)
        return image_file
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        """
        if type == 'real':
            depth_file = os.path.join(self.real_data_path,
                                      index + '-depth.png')
        elif type == 'render_real':
            depth_file = os.path.join(self.render_real_data_path,
                                      index + '-depth.png')
        elif type == 'rendered':
            depth_file = os.path.join(self.rendered_data_path,
                                      index + '-depth.png')
        if check:
            assert os.path.exists(
                depth_file), 'type:{}, Path does not exist: {}'.format(
                    type, depth_file)
        return depth_file
    ...
    def segmentation_path_from_index(self, index, type, check=True):
        ...
        """
        if type == 'real_gt':
            seg_class_file = os.path.join(self.real_data_path,
                                          index + '-label.png')
        elif type == 'rendered':
            seg_class_file = os.path.join(self.rendered_data_path,
                                          index + '-label.png')
        elif type == 'syn':
            seg_class_file = os.path.join(self.mask_syn_path,
                                          index + '-label.png')
        elif type == 'real_est':
            seg_class_file = os.path.join(self.mask_est_path,
                                          index.split('/')[0], self.cur_class,
                                          index.split('/')[1] + '-label.png')
        if check:
            assert os.path.exists(
                seg_class_file), 'type:{}, Path does not exist: {}'.format(
                    type, seg_class_file)
    ...
    def pose_from_index(self, index, type, cls_name=''):
        ...
        if type == 'real' or type == 'render_real':
            pose_file = os.path.join(self.render_real_data_path,
                                     index + '-pose.txt')
        elif type == 'rendered':
            pose_file = os.path.join(self.rendered_data_path,
                                     index + '-pose.txt')
        assert os.path.exists(
            pose_file), 'type:{}, Path does not exist: {}'.format(
                type, pose_file)
    ...
    def load_render_annotation(self, pair_index):
        ...
        pair_rec['image_real'] = self.image_path_from_index(
            pair_index[0], 'real')
        ...
        pair_rec['depth_real'] = self.depth_path_from_index(
            pair_index[0], 'real')
        ...
        if self.phase == 'train' and self.mask_syn_path != '':
            pair_rec['mask_syn'] = self.segmentation_path_from_index(
                pair_index[0], 'syn')
        pair_rec[
            'mask_idx'] = 1  # if self.phase=='train' else class_to_index[pair_rec['gt_class']]+1 # test may be not this
    ...
    def evaluate_flow(self, flow_pred, flow_gt, flow_type):
        assert len(flow_pred) == len(
            flow_gt), 'flow_pred and flow_gt length not equal'
        sum_EPE = 0
        num_inst = 0
        for i in range(len(flow_gt)):
            ...
            x_diff = (cur_flow_gt[:, :, 0] -
                      cur_flow_pred[:, :, 0])[cur_flow_gt_vis != 0]
            y_diff = (cur_flow_gt[:, :, 1] -
                      cur_flow_pred[:, :, 1])[cur_flow_gt_vis != 0]
    ...
    def evaluate_pose_add(self, config, all_poses_est, all_poses_gt,
        ...
        for cls_idx, cls_name in enumerate(self.classes):
            ...
            for iter_i in range(num_iter):
                ...
                area = simps(
                    count_correct['mean'][cls_idx, iter_i] / float(
                        count_all[cls_idx]),
                    dx=dx) / 0.1
                acc_mean = area * 100
                ...
                plt.savefig(
                    os.path.join(
                        output_dir, 'acc_thres_{}_iter{}.png'.format(
                            cls_name, iter_i + 1)),
                    dpi=fig.dpi)
        ...
        with open(
                os.path.join(output_dir, '{}_xys.pkl'.format(eval_method)),
                'wb') as f:
    ...
    def evaluate_pose_arp_2d(self, config, all_poses_est, all_poses_gt,
        ...
        for cls_idx, cls_name in enumerate(self.classes):
            ...
            for iter_i in range(num_iter):
                ...
                sum_acc_mean[iter_i] += acc_mean
                acc_02 = 100 * float(
                    count_correct['2'][cls_idx, iter_i]) / float(
                        count_all[cls_idx])
                sum_acc_02[iter_i] += acc_02
                acc_05 = 100 * float(
                    count_correct['5'][cls_idx, iter_i]) / float(
                        count_all[cls_idx])
                ...
                plt.savefig(
                    os.path.join(
                        output_dir, 'arp_2d_{}_iter{}.png'.format(
                            cls_name, iter_i + 1)),
                    dpi=fig.dpi)
