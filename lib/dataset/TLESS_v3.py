...
class TLESS_v3(IMDB):
    def __init__(self, cfg, image_set, root_path, devkit_path, class_name, result_path=None, mask_size=-1,
                 binary_thresh=None, mask_syn_name=''):
        """
        ...
        if len(mask_syn_name)>0:
            super(TLESS_v3, self).__init__('TLESS_render_v3_'+mask_syn_name, image_set, root_path, devkit_path, result_path)  # set self.name
        else:
            super(TLESS_v3, self).__init__('TLESS_render_v3', image_set, root_path, devkit_path, result_path)  # set self.name
        ...
        elif image_set.startswith('train_v1') or image_set.startswith('my_val_v1') or image_set.startswith('my_minival_v1'):
        ...
        print('TLESS_v3 rendered_data_path: {}'.format(self.rendered_data_path))
        ...
        self.config = {'comp_id': 'comp4', # obsolete items
                       'use_diff': False,  # obsolete items, use_difficult
                       'min_size': 2,
                       'render_real': cfg.dataset.RENDER_REAL}      # obsolete items, min_drop_size
    ...
    def _load_object_points(self):
        ...
        for cls_idx, cls_name in enumerate(self.classes):
            point_file = os.path.join(self._tless_path, 'models', cls_name, 'points.xyz')
    ...
    def _load_object_diameter(self):
        diameters = {}
        models_info_file = os.path.join(self._tless_path, 'models', 'models_info.yml')
