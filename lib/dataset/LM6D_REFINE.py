...
class LM6D_REFINE(IMDB):
    def __init__(self,
        ...
        """
        super(LM6D_REFINE,
              self).__init__('LM6D_refine', image_set, root_path, devkit_path,
                             result_path)  # set self.name
        self.root_path = root_path
        self.devkit_path = devkit_path
        self.gt_observed_data_path = os.path.join(devkit_path, 'data',
                                                  'gt_observed')
        ...
        print('LM6d_refine rendered_data_path: {}'.format(
        ...
        if image_set.startswith('train'):
            ...
            if len(mask_syn_name) > 0:
                ...
                'PoseCNN_val') or image_set.startswith('my_minival'):
            self.phase = 'val'
        else:
            raise Exception("unknown prefix of " + image_set)
    ...
    def _load_object_points(self):
        ...
        for cls_idx, cls_name in self.idx2class.items():
            point_file = os.path.join(self.devkit_path, 'models', cls_name,
                                      'points.xyz')
    ...
    def _load_object_diameter(self):
        diameters = {}
        models_info_file = os.path.join(self.devkit_path, 'models',
                                        'models_info.txt')
        assert os.path.exists(
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        if type == 'observed':
            ...
            depth_file = os.path.join(self.gt_observed_data_path, cls_name,
                                      index.split('/')[1] + '-depth.png')
        elif type == 'rendered':
    ...
    def pose_from_index(self, index, type, cls_name=''):
        ...
        """
        if type == 'observed' or type == 'gt_observed':
            pose_file = os.path.join(self.gt_observed_data_path, cls_name,
                                     index.split('/')[1] + '-pose.txt')
        elif type == 'rendered':
