...
class LM6D_REFINE_SYN(IMDB):
    def __init__(
        ...
        if len(mask_syn_name) > 0:
            super(LM6D_REFINE_SYN, self).__init__(
                "LM6D_data_syn_light" + mask_syn_name, image_set, root_path, devkit_path, result_path
            )  # set self.name
        else:
            super(LM6D_REFINE_SYN, self).__init__(
                "LM6D_data_syn_light", image_set, root_path, devkit_path, result_path
            )  # set self.name
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=""):
        ...
        if check:
            assert os.path.exists(image_file), "type: {}, path does not exist: {}, self.observed_data_path:{}".format(
    ...
    def load_render_annotation(self, pair_index):
        ...
        pair_rec["mask_idx"] = (
            1 if self.phase == "train" else class_to_index[pair_rec["gt_class"]] + 1
        )  # test may be not this
            ...
            for iter_i in range(num_iter):
                ...
                for i, show_idx in enumerate(show_list):
                    logger.info(
                        "{:>16}{:>8}: {:>7.2f}, {:>7.2f}, {:>7.2f}".format(
                            "average_accuracy",
                            "[{:>2}, {:>4}]".format(rot_thresh_list[show_idx], trans_thresh_list[show_idx]),
