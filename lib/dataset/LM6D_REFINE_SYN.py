...
class LM6D_REFINE_SYN(IMDB):
    def __init__(self,
        ...
        if len(mask_syn_name) > 0:
            super(LM6D_REFINE_SYN, self).__init__(
                'LM6D_data_syn_light' + mask_syn_name, image_set, root_path,
                devkit_path, result_path)  # set self.name
        else:
            super(LM6D_REFINE_SYN,
                  self).__init__('LM6D_data_syn_light', image_set, root_path,
                                 devkit_path, result_path)  # set self.name
        self.root_path = root_path
        self.devkit_path = os.path.join(devkit_path, '../LM6d_refine_syn')
        print('*******************lm6d data syn devkit path: {}'.format(
            self.devkit_path))
        self.gt_observed_data_path = os.path.join(self.devkit_path, 'data',
                                                  'gt_observed')
        self.observed_data_path = os.path.join(self.devkit_path, 'data',
                                               'observed')
        self._linemod_path = self._get_default_path()
        if image_set.startswith('train') or image_set.startswith(
        ...
        print('LM6d data syn v1 rendered path: {}'.format(
    ...
    def _load_object_points(self):
        ...
        for i in xrange(0, len(self.classes)):
            point_file = os.path.join(self._linemod_path, 'models',
                                      self.classes[i], 'points.xyz')
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        """
        if type == 'observed':
            image_file = os.path.join(self.observed_data_path,
                                      index + '-color.png')
        elif type == 'gt_observed':
            image_file = os.path.join(self.gt_observed_data_path,
                                      index + '-color.png')
        elif type == 'rendered':
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        """
        if type == 'observed':
            depth_file = os.path.join(self.observed_data_path,
                                      index + '-depth.png')
        elif type == 'gt_observed':
            depth_file = os.path.join(self.gt_observed_data_path,
                                      index + '-depth.png')
        elif type == 'rendered':
    ...
    def segmentation_path_from_index(self, index, type, check=True):
        ...
        """
        if type == 'gt_observed':
            seg_class_file = os.path.join(self.observed_data_path,
                                          index + '-label.png')
        elif type == 'rendered':
    ...
    def pose_from_index(self, index, type, cls_name=''):
        ...
        if type == 'observed' or type == 'gt_observed':
            pose_file = os.path.join(self.gt_observed_data_path,
                                     index + '-pose.txt')
        elif type == 'rendered':
    ...
    def load_render_annotation(self, pair_index):
        ...
        pair_rec['pose_observed'] = self.pose_from_index(
            pair_index[0], 'observed', cls_name=cls_name)
        pair_rec['pose_rendered'] = self.pose_from_index(
            pair_index[1], 'rendered')

        pair_rec['mask_gt_observed'] = self.segmentation_path_from_index(
            pair_index[0], 'gt_observed')
        pair_rec['mask_idx'] = 1 if self.phase == 'train' else class_to_index[
            pair_rec['gt_class']] + 1  # test may be not this
