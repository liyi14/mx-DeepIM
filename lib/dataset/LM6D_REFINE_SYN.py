...
class LM6D_REFINE_SYN(IMDB):
    def __init__(
        ...
        if len(mask_syn_name) > 0:
            super(LM6D_REFINE_SYN, self).__init__(
                "LM6D_data_syn_light" + mask_syn_name,
                image_set,
                root_path,
                devkit_path,
                result_path,
        ...
        print(
            "*******************lm6d data syn devkit path: {}".format(self.devkit_path)
        )
        self.gt_observed_data_path = os.path.join(
            self.devkit_path, "data", "gt_observed"
        )
    ...
    def segmentation_path_from_index(self, index, type, check=True):
        ...
        if check:
            assert os.path.exists(
                seg_class_file
            ), "type:{}, Path does not exist: {}".format(type, seg_class_file)
