...
class LM6D_occ_v1(IMDB):
    def __init__(self, cfg, image_set, root_path, devkit_path, class_name, result_path=None, mask_size=-1, binary_thresh=None, mask_syn_name=''):
        ...
        if len(mask_syn_name)>0:
            super(LM6D_occ_v1, self).__init__('LM6d_occ_render_v1_'+mask_syn_name, image_set, root_path, devkit_path, result_path)  # set self.name
        else:
            super(LM6D_occ_v1, self).__init__('LM6D_occ_render_v1', image_set, root_path, devkit_path, result_path)  # set self.name
        ...
        if image_set.startswith('yu_val') or image_set.startswith('yu_vis') \
                or image_set.startswith('test_vis'):
            self.rendered_data_path = os.path.join(devkit_path, 'data', 'rendered_Yu_v01')
        elif image_set.startswith('train') or image_set.startswith('vis') \
                or image_set.startswith('my_val') or image_set.startswith('my_minival'):
        ...
        if image_set.startswith('yu_val') or image_set.startswith('my_val') \
                ...
                or image_set.startswith('test_vis'):
            self.render_real_data_path = os.path.join(devkit_path, 'data', 'render_real/occ_test')


        self.mask_syn_path = ''
        if image_set.startswith('train') or image_set.startswith('vis') \
                or image_set.startswith('yu_vis'):
            self.phase = 'train'
            if len(mask_syn_name) > 0:
        ...
        self._diameters = self._load_object_diameter_from_extent()
