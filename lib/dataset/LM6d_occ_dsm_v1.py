...
class LM6D_occ_DSM_v1(IMDB):
    def __init__(self, cfg, image_set, root_path, devkit_path, class_name, result_path=None, mask_size=-1, binary_thresh=None, mask_syn_name=''):
        ...
        if len(mask_syn_name)>0:
            super(LM6D_occ_DSM_v1, self).__init__('lm6d_occ_dsm_v1'+mask_syn_name, image_set, root_path, devkit_path, result_path)  # set self.name
        else:
            super(LM6D_occ_DSM_v1, self).__init__('lm6d_occ_dsm_v1', image_set, root_path, devkit_path, result_path)  # set self.name
        self.root_path = root_path
        self.devkit_path = os.path.join(devkit_path, '../LM6d_occ_ds_multi') ################################# hard coded
        print('*******************lm6d occ dsm v1 devkit path: {}'.format(self.devkit_path))
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        if type == 'real':
            ...
            image_file = os.path.join(self.render_real_data_path, cls_name, index + '-color.png')
        elif type == 'rendered':
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        if type == 'real':
            ...
            depth_file = os.path.join(self.render_real_data_path, cls_name, index + '-depth.png')
        elif type == 'rendered':
    ...
    def segmentation_path_from_index(self, index, type, check=True):
        ...
        if type == 'real_gt':
            ...
            seg_class_file = os.path.join(self.mask_est_path, self.cur_class, index + '-label.png')  # maybe bug
        if check:
    ...
    def pose_from_index(self, index, type, cls_name=''):
        ...
        if type == 'real' or type == 'render_real':
            pose_file = os.path.join(self.render_real_data_path, cls_name, index + '-pose.txt')
        elif type == 'rendered':
