...
class LM6D_occ_DSM_v1(IMDB):
    def __init__(self,
        ...
        if len(mask_syn_name) > 0:
            super(LM6D_occ_DSM_v1, self).__init__(
                'lm6d_occ_dsm_v1' + mask_syn_name, image_set, root_path,
                devkit_path, result_path)  # set self.name
        else:
            super(LM6D_occ_DSM_v1,
                  self).__init__('lm6d_occ_dsm_v1', image_set, root_path,
                                 devkit_path, result_path)  # set self.name
        self.root_path = root_path
        self.devkit_path = os.path.join(
            devkit_path, '../LM6d_occ_ds_multi'
        )  ################################# hard coded
        print('*******************lm6d occ dsm v1 devkit path: {}'.format(
            self.devkit_path))
        self.render_real_data_path = os.path.join(self.devkit_path, 'data',
                                                  'render_real')
        self.real_data_path = os.path.join(self.devkit_path, 'data', 'real')
        self._linemod_path = self._get_default_path()
        if image_set.startswith('yu_val'):
            self.rendered_data_path = os.path.join(self.devkit_path, 'data',
                                                   'rendered_Yu_v01')
        ...
        if image_set.startswith('train'):
            self.phase = 'train'
            if len(mask_syn_name) > 0:
                self.mask_syn_path = os.path.join(self.devkit_path, 'data',
                                                  mask_syn_name)
    ...
    def _load_object_points(self):
        ...
        for cls_idx, cls_name in self.idx2class.items():
            point_file = os.path.join(self._linemod_path, 'models', cls_name,
                                      'points.xyz')
    ...
    def _load_object_diameter(self):
        diameters = {}
        models_info_file = os.path.join(self._linemod_path, 'models',
                                        'models_info.txt')
        assert os.path.exists(
    ...
    def image_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        if type == 'real':
            ...
            image_file = os.path.join(self.render_real_data_path, cls_name,
                                      index + '-color.png')
        elif type == 'rendered':
    ...
    def depth_path_from_index(self, index, type, check=True, cls_name=''):
        ...
        if type == 'real':
            ...
            depth_file = os.path.join(self.render_real_data_path, cls_name,
                                      index + '-depth.png')
        elif type == 'rendered':
    ...
    def segmentation_path_from_index(self, index, type, check=True):
        ...
        if type == 'real_gt':
            ...
            seg_class_file = os.path.join(self.mask_est_path, self.cur_class,
                                          index + '-label.png')  # maybe bug
        if check:
            assert os.path.exists(
    ...
    def pose_from_index(self, index, type, cls_name=''):
        ...
        if type == 'real' or type == 'render_real':
            pose_file = os.path.join(self.render_real_data_path, cls_name,
                                     index + '-pose.txt')
        elif type == 'rendered':
