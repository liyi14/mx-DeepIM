...
def load_cam_params(path):
    ...
    cam = {
        ...
        'K':
        np.array([[c['fx'], 0.0, c['cx']], [0.0, c['fy'], c['cy']],
                  [0.0, 0.0, 1.0]])
    }
    ...
    with open(path, 'r') as f:
        ...
        for eid in info.keys():
            if 'cam_K' in info[eid].keys():
                info[eid]['cam_K'] = np.array(info[eid]['cam_K']).reshape((3,
                                                                           3))
...
def load_ply(path):
    ...
    while True:
        line = f.readline().rstrip('\n').rstrip(
            '\r')  # Strip the newline character(s)
...
def save_ply(path,
    ...
    for pt_id, pt in enumerate(pts):
        if not np.isnan(np.sum(pt)):
            ...
            if pts_normals.size != 0:
                f.write(' ')
                f.write(
                    format_3float.format(*pts_normals[pt_id].astype(float)))
            if pts_colors.size != 0:
                f.write(' ')
    ...
    for face in faces:
        f.write(' '.join(
            map(str, map(int, [len(face)] + list(face.squeeze())))) + ' ')
        f.write('\n')
    f.close()
