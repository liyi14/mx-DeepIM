...
def get_data_pair_train_batch(pairdb, config):
    ...
    if config.network.INPUT_DEPTH:
        depth_observed, depth_rendered = get_pair_depth(pairdb, config, scale_ind_list, phase='train', random_k=random_k)
    if config.network.PRED_MASK or config.network.INPUT_MASK:
        mask_observed, mask_gt_observed, mask_rendered = get_pair_mask(pairdb, config, scale_ind_list,
    ...
    for i in range(num_pair):
        rot_i2r, trans_i2r = calc_RT_delta(pairdb[i]['pose_rendered'], pairdb[i]['pose_observed'],
        ...
        if config.train_iter.SE3_PM_LOSS:
            tgt_points = get_point_cloud_observed(config, X_obj_array[i], pose_observed=np.array(pairdb[i]['pose_observed']))
    ...
    for i in range(num_pair):
        ...
        class_index_array = my_tensor_vstack(class_index_tensor)
    data = {'image_observed': im_observed_array,
