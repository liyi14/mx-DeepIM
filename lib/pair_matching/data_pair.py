...
def get_data_pair_test_batch(pairdb, config):
    ...
    """
    im_real, im_rendered, scale_ind_list= get_pair_image(pairdb, config, 'test')
    if config.network.INPUT_DEPTH:
        depth_real, depth_rendered = get_pair_depth(pairdb, config, scale_ind_list, 'test')
    if config.network.INPUT_MASK:
        mask_real_est, _, mask_rendered = get_pair_mask(pairdb, config, scale_ind_list, 'test')

    im_info = [np.array([pairdb[i]['height'], pairdb[i]['width']], dtype=np.float32)
    ...
    for ctx_idx, data in enumerate(data_batch.data):
        ...
        for blob_idx, blob in enumerate(data):
            ...
            if blob_name.startswith('image'):
                ...
                if config.TEST.UPDATE_MASK == 'box_rendered':
                ...
                elif config.TEST.UPDATE_MASK == 'box_real_est':
                    mask_real_est = np.zeros(update_data.shape)
                    x_max = np.max(update_data, 0)
                    y_max = np.max(update_data, 1)
                    nz_x = np.nonzero(x_max)[0]
                    ...
                    if len(nz_x) == 0 or len(nz_y) == 0:
                        raise Exception("no point valid in mask_real_est")
                    ...
                    mask_real_est = update_data
                mask_update = nd.array(mask_real_est[np.newaxis, np.newaxis, :, :])
...
def get_data_pair_train_batch(pairdb, config):
    """
    return a dict of train batch
    :param pairdb: ['image_real', 'image_rendered', 'height', 'width',
                    'depth_real', 'depth_rendered', 'pose_real', 'pose_est',
                    'flipped']
    :param config: ['INTRINSIC_MATRIX', 'DEPTH_FACTOR', ...]
    :return: data: ['image_real', 'image_rendered', 'depth_real', 'depth_rendered']
             label: ['flow', 'flow_weights'(RT)]
    """
    num_pair = len(pairdb)
    random_k = np.random.randint(18)
    im_real, im_rendered, scale_ind_list = get_pair_image(pairdb, config, phase='train', random_k=random_k)
    depth_render_real = get_render_real_depth(pairdb, config, scale_ind_list, random_k=random_k)
    if config.network.INPUT_DEPTH:
        depth_real, depth_rendered = get_pair_depth(pairdb, config, scale_ind_list, phase='train', random_k=random_k)
    if config.network.PRED_MASK or config.network.INPUT_MASK:
        mask_real_est, mask_real_gt, mask_rendered = get_pair_mask(pairdb, config, scale_ind_list,
                                                               phase='train', random_k=random_k)
    im_real_array = my_tensor_vstack(im_real)
    im_rendered_array = my_tensor_vstack(im_rendered)

    depth_render_real_array = my_tensor_vstack(depth_render_real)
    if config.network.INPUT_DEPTH:
        depth_real_array = my_tensor_vstack(depth_real)
        depth_rendered_array = my_tensor_vstack(depth_rendered)
    if config.network.INPUT_MASK or config.network.PRED_MASK:
        mask_real_est_array = my_tensor_vstack(mask_real_est)
        mask_real_gt_array = my_tensor_vstack(mask_real_gt)
        mask_rendered_array = my_tensor_vstack(mask_rendered)

    if config.network.PRED_FLOW :
        flow_i2r, flow_i2r_weights, X_rendered_tensor, X_real_tensor \
    ...
    for i in range(num_pair):
        rot_i2r, trans_i2r = calc_RT_delta(pairdb[i]['pose_est'], pairdb[i]['pose_real'],
        ...
        src_pose_tensor[i] = np.array(pairdb[i]['pose_est']).reshape((1, 3, 4))
        tgt_pose_tensor[i] = np.array(pairdb[i]['pose_real']).reshape((1, 3, 4))
        if config.train_iter.SE3_PM_LOSS:
            tgt_points = get_point_cloud_real(config, X_obj_array[i], pose_real=np.array(pairdb[i]['pose_real']))
    ...
    src_pose_array = my_tensor_vstack(src_pose_tensor)
    tgt_pose_array = my_tensor_vstack(tgt_pose_tensor)
    ...
    for i in range(num_pair):
        ...
        class_index_array = my_tensor_vstack(class_index_tensor)
    data = {'image_real': im_real_array,
            'image_rendered': im_rendered_array,
            'depth_render_real': depth_render_real_array,
            'class_index': class_index_array,
            'src_pose': src_pose_array,
            'tgt_pose': tgt_pose_array}

    if config.network.INPUT_DEPTH:
        data['depth_real'] = depth_real_array
        data['depth_rendered'] = depth_rendered_array

    if config.network.INPUT_MASK:
        data['mask_real_est'] = mask_real_est_array
        data['mask_rendered'] = mask_rendered_array

    label = {'rot': rot_i2r_array,
             'trans': trans_i2r_array}

    if config.network.PRED_MASK:
        label['mask_real_gt'] = mask_real_gt_array
    ...
    if config.train_iter.SE3_PM_LOSS:
        ...
        label['point_cloud_real'] = tgt_points_array
