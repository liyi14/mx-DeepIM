...
def calc_flow(depth_src, pose_src, pose_tgt, K, depth_tgt, thresh=3E-3, standard_rep=False):
    ...
    p_within = np.logical_and(np.logical_and(pw_valid_raw >= 0, pw_valid_raw < width),
...
if __name__ == "__main__":
    ...
    idx1='000001'
    idx2='001378'
    im_src = cv2.imread('/home/yili/PoseEst/render_pangolin/synthesize/train/002_master_chef_can/{}_color.png'.format(idx1),
                        cv2.IMREAD_COLOR)
    im_tgt = cv2.imread('/home/yili/PoseEst/render_pangolin/synthesize/train/002_master_chef_can/{}_color.png'.format(idx2),
                        cv2.IMREAD_COLOR)
    depth_src = cv2.imread('/home/yili/PoseEst/render_pangolin/synthesize/train/002_master_chef_can/{}_depth.png'.format(idx1),
                           cv2.IMREAD_UNCHANGED).astype(np.float32)/10000
    depth_tgt = cv2.imread('/home/yili/PoseEst/render_pangolin/synthesize/train/002_master_chef_can/{}_depth.png'.format(idx2),
                           cv2.IMREAD_UNCHANGED).astype(np.float32)/10000
    pose_src = np.loadtxt('/home/yili/PoseEst/render_pangolin/synthesize/train/002_master_chef_can/{}_pose.txt'.format(idx1), skiprows=1)
    if True:
        from lib.pair_matching import RT_transform
        print("trans: {}".format(pose_src[:, -1]))
        print("euler: {}".format(RT_transform.mat2euler(pose_src[:,:3])))
        
    pose_tgt = np.loadtxt('/home/yili/PoseEst/render_pangolin/synthesize/train/002_master_chef_can/{}_pose.txt'.format(idx2), skiprows=1)
    ...
    for h in range(height):
        for w in range(width):
            if visible[h, w]:
                cur_flow = flow[h,w,:]
                img_src = cv2.line(img_src, (np.round(w).astype(int), np.round(h).astype(int)),
                                   (np.round(w).astype(int), np.round(h).astype(int)),
                                   (255,h*255/height,w*255/width), 5)
                img_tgt = cv2.line(img_tgt, (np.round(w + cur_flow[1]).astype(int), np.round(h + cur_flow[0]).astype(int)),
