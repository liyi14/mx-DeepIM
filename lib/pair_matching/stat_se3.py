...
def stat_se3(pairdb, config):
    ...
    num_pair = len(pairdb)
    tic()
    se3_i2r_tensor = np.zeros([num_pair, 6]) if config.network.SE3_TYPE == "EULER" else np.zeros([num_pair, 7])
    se3_i2r_dist = np.zeros([num_pair, 2])
    for i in range(num_pair):
        rot_i2r, trans_i2r = calc_RT_delta(pairdb[i]['pose_rendered'], pairdb[i]['pose_real'], np.zeros(3), np.ones(3),
                                            config.network.ROT_COORD, config.network.SE3_TYPE)
        se3_i2r_tensor[i, :-3] = rot_i2r
        se3_i2r_tensor[i, -3:] = trans_i2r

        R_dist, T_dist = calc_rt_dist_m(pairdb[i]['pose_rendered'], pairdb[i]['pose_real'])
    ...
    print("R_max: {}, T_max: {}".format(np.max(se3_i2r_dist[:, 0]), np.max(se3_i2r_dist[:, 1])))
