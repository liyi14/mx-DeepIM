...
# output_path
rendered_root_dir = os.path.join(cur_dir, '../data/LINEMOD_6D/LM6d_render_v1/data/rendered')
...
def check_real_rendered():
    ...
    for class_idx, class_name in idx2class.items():
        if class_name != 'bowl':
            continue
        print(class_name)
        real_list_path = os.path.join(real_set_root, "{}_test.txt".format(class_name))
