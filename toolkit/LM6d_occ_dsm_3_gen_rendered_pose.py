...
# output dir
pose_dir = os.path.join(LM6d_occs_syn_root, 'poses/rendered_poses')
...
def main():
    for cls_name in sel_classes:
        ...
        observed_set_file = os.path.join(observed_set_dir, "train_nd_observed_{}.txt".format(cls_name))
        with open(observed_set_file) as f:
            image_list = [x.strip() for x in f.readlines()]

        for data in tqdm(image_list):
            pose_observed_path = os.path.join(gt_observed_dir, cls_name, data+'-pose.txt')
        ...
        output_file_name = os.path.join(pose_dir, 'LM6d_occ_train_nd_rendered_pose_{}.txt'.format(cls_name))
