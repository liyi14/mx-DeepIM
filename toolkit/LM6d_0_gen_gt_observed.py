...
def check_gt_observed():
    ...
    for observed_idx in observed_indices:
        render_color_path = os.path.join(gt_observed_root, cls_name,
                                         "{}-color.png".format(observed_idx))
        render_color = cv2.imread(render_color_path)
        render_depth_path = os.path.join(gt_observed_root, cls_name,
                                         "{}-depth.png".format(observed_idx))
        render_depth = cv2.imread(render_depth_path, cv2.IMREAD_UNCHANGED)
        render_label_path = os.path.join(gt_observed_root, cls_name,
                                         "{}-label.png".format(observed_idx))
        render_label = cv2.imread(render_label_path, cv2.IMREAD_UNCHANGED)

        cls_idx = class2idx(cls_name)
        cls_idx_str = "{:02d}".format(cls_idx)
        observed_color_path = os.path.join(observed_data_root, cls_idx_str,
                                           "{}-color.png".format(observed_idx))
        observed_color = cv2.imread(observed_color_path)
        observed_depth_path = os.path.join(observed_data_root, cls_idx_str,
                                           "{}-depth.png".format(observed_idx))
        observed_depth = cv2.imread(observed_depth_path, cv2.IMREAD_UNCHANGED)
        observed_label_path = os.path.join(observed_data_root, cls_idx_str,
                                           "{}-label.png".format(observed_idx))
        ...
        plt.subplot(2, 3, 4)
        plt.imshow(
            np.int_(render_color[:, :, [2, 1, 0]] * 0.8 +
                    observed_color[:, :, [2, 1, 0]] * 0.2))

        plt.subplot(2, 3, 5)
        a = observed_depth.astype(
            np.int) * observed_label - render_depth.astype(
                np.int) * render_label
        plt.imshow(np.repeat(a[:, :, np.newaxis] + 128, 3, axis=2))

        plt.subplot(2, 3, 6)
        plt.imshow(
            np.stack([render_label, render_label, observed_label], axis=2) *
            255)
