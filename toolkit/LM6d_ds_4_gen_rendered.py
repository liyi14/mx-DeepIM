...
if __name__ == '__main__':
    ...
    rendered_pose_path = "%s/LM6d_ds_rendered_pose_{}.txt" % (os.path.join(
        LINEMOD_syn_root, 'poses', 'rendered_poses'))
    observed_list_path = "%s/LM6d_data_syn_{}_observed_{}.txt" % (os.path.join(
        LINEMOD_syn_root, 'image_set/observed'))
    ...
    for class_idx, class_name in enumerate(classes):
        ...
        for set_type in ['train']:
            ...
            rendered_per_real = 1
            assert (len(rendered_pose_list) == 1 *
                    len(real_list)), '{} vs {}'.format(
