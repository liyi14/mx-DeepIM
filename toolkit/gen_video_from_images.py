...
if __name__ == '__main__':
    ...
    images_dict = {
        k: []
        for k in ['before_ICP', 'after_ICP', 'ours', 'last_frame']
    }
    print('loading images...')
    for i in tqdm(range(N)):
        images_dict['before_ICP'].append(
            cv2.imread(
                os.path.join(before_ICP_dir, before_ICP_list[i]),
                cv2.IMREAD_COLOR))
        images_dict['after_ICP'].append(
            cv2.imread(
                os.path.join(after_ICP_dir, after_ICP_list[i]),
                cv2.IMREAD_COLOR))
        images_dict['ours'].append(
            cv2.imread(os.path.join(ours_dir, ours_list[i]), cv2.IMREAD_COLOR))
        images_dict['last_frame'].append(
            cv2.imread(
                os.path.join(last_frame_dir, last_frame_list[i]),
                cv2.IMREAD_COLOR))
    ...
    os.popen(
        ...
        .format(
            os.path.join(exp_dir,
                         'video_full/Before_After_Ours_LastFrame.avi'),
            os.path.join(
                exp_dir,
                'video_full/Before_After_Ours_LastFrame_compressed.avi')))
