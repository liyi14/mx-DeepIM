...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Generate video iter with zoom factor')
    parser.add_argument(
        '--exp_dir',
        required=True,
        help='exp_dir for generating video iter, e.g.: '
        'output/deepim_v3/deepim_v3_flownet_LM_SIXD_OCC_v1_se3_ex_u2s16_multi_all_iter_v01_zoom_RFMx4_camera2_ds_light_8epoch_4gpus/yu_val_mini_/video_iter'  # noqa:E501
    )
...
def load_object_diameter():
    ...
    with open(models_info_file, 'r') as f:
        for line in f:
            ...
            if cls_idx not in idx2class.keys():
                continue
...
def read_info(pose_info_path):
    with open(pose_info_path, 'r') as f:
        ...
        add_info = "{}: {:.2f}/{:.2f}={:.2f}%".format(add_name, add * 1000,
                                                      diameter * 1000, add_rel)
    ...
    def get_zoom_iamges(image_path,
        ...
        if use_first_zoom_factor:
            init_info_path = image_path.replace(
                os.path.basename(image_path)[os.path.basename(image_path).
                                             find('iter'):],
                'iter_00_info.txt')
        ...
        # print(zoom_factor)
        image_real = cv2.imread(image_path, cv2.IMREAD_COLOR).transpose(
            [2, 0, 1])[None, :, :, :]
