...
if __name__ == '__main__':
    class_name_list = [
        '__back_ground__', 'ape', 'benchvise', 'camera', 'can', 'cat',
        'driller', 'duck', 'eggbox', 'glue', 'holepuncher', 'iron', 'lamp',
        'phone'
    ]
    big_idx2class = {
        1: 'ape',
    ...
    keyframe_path = "%s/{}_test.txt" % (os.path.join(LINEMOD_root,
                                                     'image_set/observed'))
    exMethod_pred_dir = os.path.join(
        cur_path,
        '../data/LINEMOD_6D/LM6d_converted/LM6d_refine/{}_LINEMOD_6D_results'.
        format(version))

    # output_path
    rendered_root_dir = os.path.join(LINEMOD_root,
                                     'data/rendered_val_{}'.format(version))
    ...
    for small_class_idx, class_name in enumerate(class_name_list):
        ...
        for idx, observed_index in enumerate(tqdm(observed_index_list)):
            ...
            if labels != -1:
                ...
                pose_icp_q = exMethod_pred['poses_icp'][proposal_idx].reshape(
                    7)
                ...
                pose_ori_file = os.path.join(
                    rendered_dir, '{}_{}_{}-pose.txt'.format(
                        class_name, observed_prefix_list[idx], 0))
                pose_icp_file = os.path.join(
                    rendered_dir, '{}_{}_{}-pose_icp.txt'.format(
                        class_name, observed_prefix_list[idx], 0))
                image_file = os.path.join(
                    rendered_dir, '{}_{}_{}-color.png'.format(
                        class_name, observed_prefix_list[idx], 0))
                depth_file = os.path.join(
                    rendered_dir, '{}_{}_{}-depth.png'.format(
                        class_name, observed_prefix_list[idx], 0))
                segmentation_file = os.path.join(
                    rendered_dir, '{}_{}_{}-label.png'.format(
                        class_name, observed_prefix_list[idx], 0))
                ...
                all_pair.append("{} {}/{}/{}_{}_{}".format(
                    observed_index, video_name_list[idx], class_name,
                    class_name, observed_prefix_list[idx], 0))
            else:
                print("no exMethod_pred in {} {} {}".format(
