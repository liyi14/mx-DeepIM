...
from lib.pair_matching.RT_transform import (mat2euler, euler2mat, euler2quat,
                                            calc_rt_dist_m)
from math import pi
...
for cls_idx, cls_name in enumerate(sel_classes):
    ...
    for observed_idx in tqdm(image_list):
        pose_observed_path = os.path.join(data_dir,
                                          '{}-pose.txt'.format(observed_idx))
        ...
        for rendered_idx in range(num_rendered_per_observed):
            ...
            while (r_dist > angle_max
                ...
                if count == 100:
                    ...
                    print("count: {}, image_path: {}, rendered_idx: {}".format(
                        count,
