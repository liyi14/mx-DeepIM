...
if __name__ == '__main__':
    ...
    for i in range(len(z_image_real_dirs)):
        ...
        rendered_files = [os.path.join(z_image_rendered_dir, fn) for fn in os.listdir(z_image_rendered_dir) if '.png' in fn]
    ...
    for i in tqdm(range(N)):
        images_dict['pose'].append(cv2.imread(pose_path_list[i],
                                            cv2.IMREAD_COLOR))
        images_dict['zoom_mask'].append(cv2.imread(zoom_mask_path_list[i],
                                              cv2.IMREAD_COLOR))
        images_dict['zoom_image_real'].append(cv2.imread(z_image_real_path_list[i],
                                              cv2.IMREAD_COLOR))
        images_dict['zoom_image_rendered'].append(cv2.imread(z_image_rendered_path_list[i],
                                              cv2.IMREAD_COLOR))
    ...
    video_pose_mask_image = cv2.VideoWriter(os.path.join(exp_dir, '../video_full/zoom_image_mask_pose.avi'), fourcc, 2.0, (width, height))
