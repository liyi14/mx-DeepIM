...
if __name__ == '__main__':
    ...
    z_image_real_dirs = [
        os.path.join(exp_dir, d) for d in os.listdir(exp_dir)
        if 'zoom_image_real' in d
    ]
    z_image_real_dirs = sorted(z_image_real_dirs)

    z_image_rendered_dirs = [
        os.path.join(exp_dir, d) for d in os.listdir(exp_dir)
        if 'zoom_image_rendered' in d
    ]
    ...
    for z_mask_dir in zoom_mask_dirs:
        files = [
            os.path.join(z_mask_dir, fn) for fn in os.listdir(z_mask_dir)
            if '.png' in fn
        ]
        files.sort(cmp=my_cmp)
        for i in range(len(files)):
    ...
    for i in range(len(z_image_real_dirs)):
        ...
        real_files = [
            os.path.join(z_image_real_dir, fn)
            for fn in os.listdir(z_image_real_dir) if '.png' in fn
        ]
        rendered_files = [
            os.path.join(z_image_rendered_dir, fn)
            for fn in os.listdir(z_image_rendered_dir) if '.png' in fn
        ]
    ...
    images_dict = {
        k: []
        for k in [
            'flow', 'pose', 'mask', 'zoom_mask', 'zoom_image_real',
            'zoom_image_rendered'
        ]
    }
    ...
    for i in tqdm(range(N)):
        images_dict['pose'].append(
            cv2.imread(pose_path_list[i], cv2.IMREAD_COLOR))
        images_dict['zoom_mask'].append(
            cv2.imread(zoom_mask_path_list[i], cv2.IMREAD_COLOR))
        images_dict['zoom_image_real'].append(
            cv2.imread(z_image_real_path_list[i], cv2.IMREAD_COLOR))
        images_dict['zoom_image_rendered'].append(
            cv2.imread(z_image_rendered_path_list[i], cv2.IMREAD_COLOR))
    ...
    for i in tqdm(range(N)):
        ...
        res_img = np.vstack((np.hstack((res_img_1, res_img_2)),
                             np.hstack((res_img_3, res_img_4))))
        im_scale = 0.5
        res_img = cv2.resize(
            res_img,
            None,
    ...
    os.popen(
        ...
        .format(
            ...
            os.path.join(exp_dir,
                         '../video_full/zoom_image_mask_pose_compressed.avi')))
