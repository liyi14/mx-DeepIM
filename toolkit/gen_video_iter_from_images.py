...
if __name__ == '__main__':
    ...
    flow_dirs = [
        os.path.join(exp_dir, d) for d in os.listdir(exp_dir) if 'flow' in d
    ]
    ...
    mask_dirs = [
        os.path.join(exp_dir, d) for d in os.listdir(exp_dir) if 'mask' in d
    ]
    ...
    for flow_dir in flow_dirs:
        files = [
            os.path.join(flow_dir, fn) for fn in os.listdir(flow_dir)
            if '.png' in fn
        ]
    ...
    for mask_dir in mask_dirs:
        files = [
            os.path.join(mask_dir, fn) for fn in os.listdir(mask_dir)
            if '.png' in fn
        ]
        files.sort(cmp=my_cmp)
        for i in range(len(files)):
    ...
    for i in range(N_mask):
        if len(mask_path_list) != 0:
            images_dict['mask'].append(
                cv2.imread(mask_path_list[i], cv2.IMREAD_COLOR))

    for i in tqdm(range(N)):
        if len(flow_path_list) != 0:
            images_dict['flow'].append(
                cv2.imread(flow_path_list[i], cv2.IMREAD_COLOR))
    ...
    if len(mask_path_list) != 0:
        video_mask = cv2.VideoWriter(
            os.path.join(exp_dir, '../video_full/mask.avi'), fourcc, 5.0,
            (width, height))

    if len(flow_path_list) != 0:
        video_flow = cv2.VideoWriter(
            os.path.join(exp_dir, '../video_full/flow.avi'), fourcc, 5.0,
            (width, height))
        video = cv2.VideoWriter(
            os.path.join(exp_dir, '../video_full/pose_flow.avi'), fourcc, 5.0,
            (width * 2, height))
    video_pose = cv2.VideoWriter(
        os.path.join(exp_dir, '../video_full/pose.avi'), fourcc, 10.0,
        (width, height))
        ...
        os.popen(
            ...
            .format(
                ...
                os.path.join(exp_dir,
                             '../video_full/pose_flow_compressed.avi')))
