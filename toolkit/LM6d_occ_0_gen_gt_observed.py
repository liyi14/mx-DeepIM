...
observed_set_dir = os.path.join(LM6d_occ_root, 'image_set/observed')
observed_data_root = os.path.join(LM6d_occ_root, 'data/observed')
gt_observed_root = os.path.join(LM6d_occ_root, 'data/gt_observed')
...
def gen_gt_observed():
    for cls_idx, cls_name in idx2class.items():
        ...
        # render machine
        model_dir = os.path.join(LM6d_occ_root, 'models', cls_name)
        render_machine = Render_Py(model_dir, K, width, height, ZNEAR, ZFAR)
        for set_type in set_type_list:
            with open(os.path.join(observed_set_dir, '{}_{}.txt'.format(cls_name, set_type)), 'r') as f:
            ...
            for observed_idx in tqdm(all_indices):
                ...
                if set_type == 'train':
                    gt_observed_dir = os.path.join(gt_observed_root, cls_name)
                elif set_type == 'val':
                    gt_observed_dir = os.path.join(gt_observed_root, 'val', cls_name)
                new_pose_path = os.path.join(gt_observed_dir, "{}-pose.txt".format(prefix))
                mkdir_if_missing(os.path.dirname(new_pose_path))
                # write pose
                ...
                if any([x in observed_idx for x in ['000128', '000256', '000512']]):
                    ...
                    render_color_path = os.path.join(gt_observed_dir, "{}-color.png".format(prefix))
                    cv2.imwrite(render_color_path, rgb_gl)

                # depth
                depth_save = depth_gl * DEPTH_FACTOR
                depth_save = depth_save.astype('uint16')
                render_depth_path = os.path.join(gt_observed_dir, "{}-depth.png".format(prefix))
                ...
                # write label
                label_path = os.path.join(gt_observed_dir, "{}-label.png".format(prefix))
