...
if __name__=='__main__':
    ...
    keyframe_path = "%s/{}_val.txt"%(os.path.join(cur_path, '../data/LINEMOD_6D/LM6d_occ_render_v1/image_set/real'))
    yu_pred_dir = os.path.join(cur_dir, '../data/LINEMOD_6D/results_occlusion_posecnn')
    ...
    real_root_dir = os.path.join(cur_path, '..', 'data', 'LINEMOD_6D', 'LM6d_occ_render_v1', 'data', 'real')
    real_meta_path = "%s/{}-meta.mat" % (real_root_dir)
    rendered_root_dir = os.path.join(cur_path, '..', 'data', 'LINEMOD_6D', 'LM6d_occ_render_v1', 'data', version)
    pair_set_dir = os.path.join(cur_path, '..', 'data', 'LINEMOD_6D', 'LM6d_occ_render_v1', 'image_set')
    ...
    for small_class_idx, class_name in enumerate(class_name_list):
        ...
        # init render
        model_dir = os.path.join(cur_dir, '../data/LINEMOD_6D/models/{}'.format(class_name))
        if gen_images:
        ...
        for idx, real_index in enumerate(real_index_list):
            ...
            yu_idx = int(real_prefix_list[idx]) - 1
            yu_pred_file = os.path.join(yu_pred_dir, "{:04d}.mat".format(yu_idx))
            ...
            if len(labels) >= 1:
                ...
                if len(proposal_idx) == 1:
                    ...
                    pose_ori_m = se3.se3_q2m(pose_ori_q)
                    pose_ori_q[:4] = se3.mat2quat(pose_ori_m[:, :3])  # ??

                    pose_icp_m = se3.se3_q2m(pose_icp_q)
                    pose_icp_q[:4] = se3.mat2quat(pose_icp_m[:, :3])

                    pose_gt = meta_data['poses']
                    if len(pose_gt.shape)>2:
                        pose_gt = pose_gt[:, :, list(meta_data['cls_indexes'][0]).index(big_class_idx)]
                    print("{}, {:04d}, {}".format(class_name, yu_idx, se3.calc_rt_dist_m(pose_ori_m, pose_gt)))

                    pose_ori_file = os.path.join(rendered_dir,
                                                 '{}_{}_{}-pose.txt'.format(class_name, real_prefix_list[idx], 0))
                    pose_icp_file = os.path.join(rendered_dir,
                                                 '{}_{}_{}-pose_icp.txt'.format(class_name, real_prefix_list[idx], 0))
                ...
                print("no yu_pred in {} {} {}".format(class_name, yu_idx, real_index))
