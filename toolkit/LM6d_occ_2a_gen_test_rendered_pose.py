...
real_set_dir = os.path.join(cur_dir, '../data/LINEMOD_6D/LM6d_occ_render_v1/image_set/real')
real_data_root = os.path.join(cur_dir, '../data/LINEMOD_6D/LM6d_occ_render_v1/data/real')
LM6d_root = os.path.join(cur_dir, '../data/LINEMOD_6D')

# render real
render_real_root = os.path.join(cur_dir, '../data/LINEMOD_6D/LM6d_occ_render_v1/data/render_real/occ_test')
...
for cls_idx, cls_name in idx2class.items():
    ...
    for real_idx in image_list:
        ...
        for rendered_idx in range(num_rendered_per_real):
            ...
            count = 0
            while (r_dist>45 or not(16<center_x<(640-16) and 16<center_y<(480-16))):
                tgt_euler = src_euler + np.random.normal(0, 15.0/180*pi, 3)
                x_error = np.random.normal(0, 0.01, 1)[0]
                y_error = np.random.normal(0, 0.01, 1)[0]
                z_error = np.random.normal(0, 0.05, 1)[0]
    ...
    output_file_name = os.path.join(pose_dir, 'LM6d_occ_{}_{}_test_rendered_pose_{}.txt'.format(version, image_set, cls_name))
