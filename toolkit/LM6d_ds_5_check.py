...
def check():
    ...
    for pair in pairs:
        ...
        gt_rendered_color = read_img(  # noqa:F841
            os.path.join(gt_observed_dir, "{}-color.png".format(observed_idx)),
            3)
        ...
        plt.subplot(2, 3, 5)
        observed_rendered_diff = np.abs(observed_color[:, :, [2, 1, 0]] -
                                        rendered_color[:, :, [2, 1, 0]])
...
def check_rot_distribution():
    ...
    for cls_idx, cls_name in idx2class.items():
        ...
        train_idx_file = os.path.join(
            observed_set_dir,
            "LM6d_data_syn_train_observed_{}.txt".format(cls_name))
...
def check_model_points():
    ...
    for cls_idx, cls_name in idx2class.items():
        ...
        for observed_i, observed_idx in enumerate(tqdm(observed_indices)):
            ...
            pose_path = os.path.join(gt_observed_dir, cls_name,
                                     '{}-pose.txt'.format(prefix))
            observed_color_path = os.path.join(gt_observed_dir, cls_name,
